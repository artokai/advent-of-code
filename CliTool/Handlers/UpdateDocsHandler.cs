...
public class UpdateDocsHandler
{
    ...
    private void WriteYearlyMarkdown(int year, Dictionary<int, PuzzleInfo> puzzles)
    {
        ...
        for (var day = 1; day <= 25; day++) {
            ...
            var partA = puzzles.ContainsKey(day) ? $"[Part 1](D{day:D2}/{puzzles[day].PartA})" : "";
            var partB = puzzles.ContainsKey(day) ? $"[Part 2](D{day:D2}/{puzzles[day].PartB})" : "";
        ...
        foreach (var line in lines) {
            if (line.StartsWith("## Year")) {
                ...
                for (var day = 1; day <= 25; day++) {
                    ...
                    var partA = puzzles[year].ContainsKey(day) ? $"[Part 1](Puzzles/Y{year}/D{day:D2}/{puzzles[year][day].PartA})" : "";
                    var partB = puzzles[year].ContainsKey(day) ? $"[Part 2](Puzzles/Y{year}/D{day:D2}/{puzzles[year][day].PartB})" : "";
                    sb.Append($"| {day} | [{title}](https://adventofcode.com/{year}/day/{day}) | {partA} | {partB} |\n");
                }
                sb.Append('\n');
            }
