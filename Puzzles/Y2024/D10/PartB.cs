...
[PuzzleInfo(year: 2024, day: 10, part: 2, title: "Hoof It")]
public class PartB : SolverBase
{
    ...
    private int FindPath(int[,] map, int x, int y, int indent = 0)
    {
        var current = map[x, y];
        if (current == 9)
        {
        ...
        if (x + 1 < map.GetLength(0) && map[x + 1, y] == current + 1)
        {
            ...
            result += FindPath(map, x + 1, y, indent);
        }
        ...
        if (y + 1 < map.GetLength(1) && map[x, y + 1] == current + 1)
        {
            ...
            result += FindPath(map, x, y + 1, indent);
        }
